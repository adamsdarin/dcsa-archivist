...
def build_directive_splits(root: Path, records: list[dict[str, Any]]
    ...
    for name, spec in DIRECTIVES.items():
        ...
        manifest = {"directive": name, "source_file": src.name,
                    ...
                    "generated_by": "dcsa-archivist build-candidate "
                                     "(src/dcsa_custodian/directive_splits.py)",
        ...
        readme = (
            ...
            f"`../{src.name}` with no loss, as part of a DCSA Archivist release. "
            ...
            + ("**Load `02_Appendix_A_Introduction_and_Adjudicative_Process.md` "
               ...
               "through the letter.\n\n")
            + f"| File | Contents |\n|---|---|\n{index}\n\n"
            f"Regenerated by `python custodian.py build-candidate` — never edit "
